...
def get_SciHub_pdf_url(article_url, file):
    """
    It can be deprecated when you have access to the Springer, Acm, IEEE.
    If not have access to Spring, Acm, IEEE then you can try to use the function.
    :param article_url: url of the target article
    :return:
    """
    # convert acm articles to Sci-Hub articles
    # if the articles from other lib, the next line of code need to change!
    # get the sci url.
    sci_url = "https://sci-hub.se/" + article_url.split("/")[5] + '/' + article_url.split("/")[6]
    ...
    if response.status_code == 200:
        ...
        if download_button:
            ...
            matches = re.findall(pattern, onclick_value)

            if matches == None:
                print("the article can not download from Sci-Hub either")
                file.write(article_url + '\n')
                return None

            pdf_url = "https://sci-hub.se" + matches[0]
            return pdf_url
    else:
        file.write(article_url + '\n')
        print("fail to fetch the article url", article_url)

def download_pdf_SciHub(url_list, from_, store_to_path, file):
    """
    download articles from Sci-Hub web.
    Pay attention! the article url from Acm, Springer, IEEE is not the same,
    so you need to the way to get the download url.
    :param url_list: article url list
    :param from_: starting index
    :param store_to_path: where to store the articles.
    :return: None
    """
    len_url_list = len(url_list)
    print("need to be download in total:", len_url_list)
    for i in range(from_, len_url_list):
        pdf_url = get_SciHub_pdf_url(url_list[i], file)
        if pdf_url != None:
            # get the pdf name.
            pdf = store_to_path + url_list[i].split("/")[5] + '_' + url_list[i].split("/")[6] + ".pdf"
            response = requests.get(pdf_url)
            if response.status_code == 200:
                # if can not request then write the article url into a '.txt' or '.csv' file.
                with open(pdf, "wb") as f:
                    f.write(response.content)
                print("pdf download number:", str(i+1))
            else:
                print("failed to fetch", pdf_url)
                file.write(url_list[i] + "\n")
        else:
            file.write(url_list[i] + "\n")
            print("number %d the article can not be downloaded!url:" % (i + 1), url_list[i])
